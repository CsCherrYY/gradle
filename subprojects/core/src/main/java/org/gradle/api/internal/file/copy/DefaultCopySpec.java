...
public class DefaultCopySpec implements CopySpecInternal {
    ...
    public class DefaultCopySpecResolver implements CopySpecResolver {
        ...
        @Override
        public Integer getFileMode() {
            return getMode(getFileReadOnlyPermissions());
        }

        @Override
        public Integer getDirMode() {
            return getMode(getDirReadOnlyPermissions());
        }

        @Nullable
        private Integer getMode(Provider<ReadOnlyFileAccessPermissions> permissions) {
            return permissions.isPresent() ? ((FileAccessPermissionsInternal) permissions.get()).toUnixNumeric() : null;
        }

        @Override
        public Provider<ReadOnlyFileAccessPermissions> getFileReadOnlyPermissions() {
            return getPermissions(filePermissions, CopySpecResolver::getFileReadOnlyPermissions);
        }

        @Override
        public Provider<ReadOnlyFileAccessPermissions> getDirReadOnlyPermissions() {
            return getPermissions(dirPermissions, CopySpecResolver::getDirReadOnlyPermissions);
        }

        private Provider<ReadOnlyFileAccessPermissions> getPermissions(Property<FileAccessPermissions> property, Function<CopySpecResolver, Provider<ReadOnlyFileAccessPermissions>> parentMapper) {
