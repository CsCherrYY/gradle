...
public class DefaultFileCopyDetails extends AbstractFileTreeElement implements FileVisitDetails, FileCopyDetailsInternal {
    ...
    public Provider<ReadOnlyFileAccessPermissions> getReadOnlyPermissions() {
        if (permissions != null) {
            ...
            return Providers.of(new DefaultReadOnlyFileAccessPermissions(permissions.get().toUnixNumeric()));
        }

        Integer specMode = getSpecMode();
        if (specMode != null) {
            return Providers.of(new DefaultReadOnlyFileAccessPermissions(specMode));
        }

        return Providers.of(new DefaultReadOnlyFileAccessPermissions(fileDetails.getMode()));
    }
